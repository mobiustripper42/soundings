// Ed25519 verification, derived from TweetNaCl (public domain).
// See ed25519.h for why this is vendored rather than linked against libsodium.
//
// ⚠ EDIT THIS FILE ONLY TO TRACK UPSTREAM. The body below is deliberately kept
// in TweetNaCl's shape — same function names, same limb layout, same terse
// arithmetic — so a reviewer can diff it against https://tweetnacl.cr.yp.to
// rather than audit field arithmetic from first principles. Reformatting it to
// match this repo's style would destroy the only cheap way to check it.
//
// Only the verification path is here. crypto_sign, the key-generation path, and
// the secretbox/box primitives are all omitted: nothing on the node signs.
...
// Upstream writes `static const gf gf0;` — legal C, but C++ requires a const object to
// be initialized. The explicit zero is the only deviation from TweetNaCl's text in this
// file, and it is a spelling change rather than a behavioural one.
...
static void ts64(u8* x, u64 u) {
    int i;
    for (i = 7; i >= 0; --i) { x[i] = (u8)(u & 0xff); u >>= 8; }
}
...
static int vn(const u8* x, const u8* y, int n) {
    int i;
    unsigned int d = 0;
    FOR(i, n) d |= (unsigned int)(x[i] ^ y[i]);
    return (1 & ((d - 1) >> 8)) - 1;
}
...
// --- Reduction mod the group order L ---------------------------------------
...
bool verifyEd25519(const uint8_t* msg, size_t msgLen,
    ...
    // Bounded before anything is copied. The buffer below is a fixed frame and
    // this is what keeps a long message from reaching it.
    if (msg == nullptr || sig == nullptr || pubkey == nullptr) return false;
    if (msgLen > kMaxSignedMessageBytes) return false;

    gf p[4], q[4];
    if (unpackneg(q, pubkey)) return false;    // not a point on the curve

    // SHA-512(R || A || M), per RFC 8032 §5.1.7.
    u8 buf[64 + kMaxSignedMessageBytes];
    memcpy(buf, sig, 32);
    memcpy(buf + 32, pubkey, 32);
    memcpy(buf + 64, msg, msgLen);

    u8 h[64];
    crypto_hash(h, buf, 64 + (u64)msgLen);
    reduce(h);
