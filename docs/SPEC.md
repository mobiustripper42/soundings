...
> **Amended by DEC-012 — the Not-V1 row forbidding OTA; the rest of scope is unchanged**
<!-- /amended-by-dec -->
...
- ~~**OTA firmware updates** — V1 reflashes over USB during the annual service
  ...
  **Built in Phase 3.9d (DEC-012).** The injunction was against building it *early*, and
  it held for eight phases. By 3.9d there was firmware worth updating and a node destined
  for a tank lid, so the condition it named had been met. USB remains the recovery path
  and OTA is a convenience on top of it, never a replacement — there is no rollback.
...
<!-- amended-by-dec: generated by `npm run gen:decisions` — do not edit by hand -->
> **Amended by DEC-006 — the per-node component list — the cell line, plus the enclosure size, gland size and SMA-bulkhead rows that Round 3 sourcing corrected — and the 15-minute cadence, which becomes a floor rather than a preference; the firmware-cutoff half of this decision is unchanged**
<!-- /amended-by-dec -->
...
| ~~2× 18650 cells, parallel, **protected**~~ **1S2P 18650 pack, protected** | **Changed 2026-08-21 — DEC-006 amendment.** A single pre-built **1S2P pack, 3.7 V, 5200 mAh nameplate, integrated PCM**. Three bought: one in service, one charged, one spare. Replaces the loose cells, the holder **and** the separate charger — a factory pack is matched at build and never separated, so DEC-006's equalise-before-joining requirement is met by construction. ⚠ **5200 mAh is nameplate, not measured** — the listing is demonstrably unreliable, and VBAT telemetry is what will confirm it. ⚠ The holder's parallel-not-series trap is gone with the holder. |
...
| IP65 enclosure | Light-coloured, hinged lid. **~150 × 100 × 70 mm** — ⚠ **corrected 2026-08-21; the old ~4×4×2" was too small, and the shortfall was DEPTH.** The U.FL pigtail's ~10 mm bend radius is what consumes internal depth, not the components. Light colour is a **cell** spec, not aesthetics. |
| Wago lever-nut terminal block | Internal, serviceable sensor connections |
| Cable glands, size TBD at build | One per cable entry, pointing down. ⚠ **PG7 was wrong** — it clamps ~3–6.5 mm, under the actual cable OD. Size against the real cable; assorted sizes are on hand. |
| U.FL→SMA-female **bulkhead pigtail** + 4 dBi whip antenna | Through the enclosure wall. ⚠ **The bulkhead is not a separate part** — it is the SMA end of the pigtail. Ordering both yields two pigtails and two unusable connectors. |
...
> a comfortable call at 6000 mAh (10 min dropped the margin to ~1.3×). **It is
> no longer comfortable.** Under the 2026-08-21 DEC-006 amendment the pack is
> **5200 mAh nameplate**, so cold-derated capacity is ~3,640 mAh against a
> ~2,460 mAh two-year draw — **~1.48× margin at 15 minutes**. At 10 minutes the
> wake term grows from ~1,750 to ~2,625 mAh and the margin lands near **1.1×**,
> which is no margin at all. Tank level does not need 10-minute resolution and
> the battery budget can no longer pay for it. Full budget in
...
<!-- amended-by-dec: generated by `npm run gen:decisions` — do not edit by hand -->
> **Amended by DEC-012 — the updates line only; the one-firmware and host-testable-core points stand**
<!-- /amended-by-dec -->
...
- **Updates:** USB flashing at deploy/service time, **plus OTA over WiFi triggered on
  the radio downlink (DEC-012)**. WiFi stays disabled except during an update, so the
  "WiFi disabled" point above still holds for every other moment of the node's life.
  ⚠ No rollback: a valid-but-broken image is recovered with a cable.
