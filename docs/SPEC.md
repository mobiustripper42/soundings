...
| D4 | **Gateway box** | Pi Zero 2 W vs Heltec LoRa→WiFi bridge vs radio-on-the-server. Central/on-LAN placement removes the antenna-reach pressure that would force this. **Provisionally answered 2026-08-08: bee-grace (the existing headless Beelink) in a second-floor finished room, ~25–30 ft above the tank cluster**, with the gateway radio USB-tethered beside it. That siting clears the Fresnel requirement outright and removes the heat, cable-length, and WiFi-coverage pressures that would have forced a separate box or a WiFi bridge. **Not closed until the Phase 3.2 range test confirms the link from that position** — see `docs/HARDWARE_BUILD_PLAN.md` §3. |
...
| D7 | **Node-ID → location mapping** | Config file vs DB table. Designed with the gateway. **Now scheduled: Phase 3.6 (DEC-005)** — the tank node is the first thing that needs a location. |
...
| D9 | **MQTT topic hierarchy** | Finalized with the gateway/ingestion. **Root constrained to `farm/soundings/…` (DEC-005)**, resolving a conflict with the `farm/water/cluster/*` namespace in `docs/tank-level-sensor.md`. The hierarchy below the root is still designed in Phase 3.6. |
