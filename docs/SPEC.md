...
> **Amended by DEC-009 — the D3 and D4 rows, both now resolved; the rest of the register is unchanged**
<!-- /amended-by-dec -->
...
| D3 | **Gateway radio chip / node↔gateway PHY pairing** | SX1262 (node) vs SX127x/RFM95 (gateway). Behind an adapter, faked in sim; decided at the bench. *(Hardware-affects-software — evaluated and found safely deferrable.)* **Now scheduled: Phase 3.2 range test (DEC-005).** Leading candidate is a second Heltec V3 as the gateway radio — SX1262 both ends dissolves the pairing question entirely. See `docs/HARDWARE_BUILD_PLAN.md` §3. |
| D4 | **Gateway box** | Pi Zero 2 W vs Heltec LoRa→WiFi bridge vs radio-on-the-server. Central/on-LAN placement removes the antenna-reach pressure that would force this. **Now scheduled: Phase 3.2 (DEC-005)** — the same range test decides it, since the answer is "wherever the link closes." |
...
| ~~D7~~ | **Node-ID → location mapping** — RESOLVED 2026-08-10 → **see DEC-008** | A **config file**, not a DB table: the map is what tells ingestion where a reading belongs, so it must be readable before anything is written to the database, and a table would couple provisioning to whichever store D6 picks. Lives at `gateway/config/soundings.toml`, keyed by node id with a `location` and a `role`. Built in Phase 3.6 (issue #45). |
| D8 | **TimescaleDB schema + JOIN shape to farm records** | Designed with ingestion. Poop Deck's to own under DEC-004; soundings' side is the publish shape (Phase 3.7). |
| ~~D9~~ | **MQTT topic hierarchy** — RESOLVED 2026-08-10 → **see DEC-008** | Two branches under the `farm/soundings/…` root DEC-005 fixed: `node/<node_id>/reading` (the decoded packet as JSON — keyed by hardware, the audit trail) and `<location>/<metric>` (derived scalars — keyed by place, one number per topic). Built in Phase 3.6 (issue #45). |
