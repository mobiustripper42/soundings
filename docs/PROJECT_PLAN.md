...
| 3.8 | Real A02YYUW UART driver behind `IDistance` — free-running frame parse, checksum, discard-first-3 + **median of 5–7**. Plus the **DS18B20 headspace read** (DEC-007) and the switched sensor rail — **Vext/GPIO36 directly; the P-FET is no longer needed** (HW-18 measured 3.0 mV 2026-08-20, F-6 retired). ⚠ Scope shrank after estimation; the 3 may now be light. | 3 | [#47](https://github.com/mobiustripper42/soundings/issues/47) |
