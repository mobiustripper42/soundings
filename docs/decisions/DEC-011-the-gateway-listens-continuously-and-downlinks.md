...
**Revisit — and the first item is nearer than it looks:**

⚠ **The reply window does not poll the radio, and that is a software gap, not just an RF one.** For up to 400 ms after relaying a packet the gateway polls only the serial port. The chip stays in receive, but `RX_TIMEOUT_INF` auto-restarts into the **same buffer**, so a packet landing while an unread one sits there overwrites it rather than queueing. With one node this cannot happen — the only transmitter in earshot is the node holding its window, and it then sleeps for fifteen minutes. **`SPEC.md` §13 schedules 2–3 nodes in Red Tunnel as the next rollout step**, and this starts costing packets there, *before* any RF collision does. Resolve it before the multi-node bench: poll the radio inside the wait, shrink the window, or accept a measured loss rate deliberately. Raised by `@code-review` on the 3.9c PR, which was right that the code comment stated the single-node case as settled fact rather than as the bench simplification it is.

Beyond that: two nodes transmitting at once is genuinely unaddressed here. And if a downlink is ever needed that cannot be expressed as a state, this decision is what it has to argue against.
