...
| HW-15 | **Heltec Wireless Stick Lite V3** — does it share the WiFi LoRa 32 V3's pinout for the load-bearing pins (Vext control, ADC_Ctrl + battery-sense divider, SX1262 SPI)? Same U.FL connector? Same JST 1.25 battery connector? | Proposed as the **node** board (no OLED, ~$10 cheaper). If the pinout differs, HW-02's disable list and HW-05/HW-14's answers **do not transfer** and Round 1 gets partly re-run. | `open` | | |
